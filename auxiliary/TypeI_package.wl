...
(*uncomment when TypeI_script.wls is run in manual mode*)
...
(*Format of the vector of parameters = {g,\[CapitalDelta],Subscript[c, 1,2],Subscript[c, 1,4],...,Subscript[c, 1,cutP],Subscript[c, 2,2],Subscript[c, 2,4],...,Subscript[c, 2,cutP],Subscript[c, 3,2],Subscript[c, 3,4],...,Subscript[c, 3,cutP],Subscript[c, 4,2],Subscript[c, 4,4],...,Subscript[c, 4,cutP],}*)

\[DoubleStruckCapitalF]V0[Plist_List]:=Block[{},
If[Length[Flatten[Plist]]!=2cutP+2,Print["Length error"];Return[];];
sb0=({
c[2,-(powP[[1]]-powP[[2]])]->0,
c[3,-(powP[[1]]-powP[[3]])]->0,
c[3,-(powP[[2]]-powP[[3]])]->0,
...
Sequence@@Table[c[1,i]->Plist[[2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[c[2,i]->Plist[[2 + cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[c[3,i]->Plist[[2 + cutP/2 +cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[c[4,i]->Plist[[2 + cutP/2 + cutP/2 + cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}]
...
IndepParams={\[CapitalDelta],Table[c[1,i],{i,2,cutP,2}],Table[c[2,i],{i,2,cutP,2}],Table[c[3,i],{i,2,cutP,2}],Table[c[4,i],{i,2,cutP,2}]}/.c[2,-(powP[[1]]-powP[[2]])]->0/.c[3,-(powP[[1]]-powP[[3]])]->0/.c[3,-(powP[[2]]-powP[[3]])]->0/.c[4,-(powP[[1]]-powP[[4]])]->0//Flatten//DeleteCases[#,0]&;
ParamsInPlist={g,\[CapitalDelta],Table[c[1,i],{i,2,cutP,2}],Table[c[2,i],{i,2,cutP,2}],Table[c[3,i],{i,2,cutP,2}],Table[c[4,i],{i,2,cutP,2}]}//Flatten;
...
If[Length[Flatten[Plist]]==2cutP+2,
coefs = Plist;
Return[coefs]
];
If[Length[Flatten[Plist]]!=2cutP+2,
...
lstc1=Take[lstc1~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstc2=Take[lstc2~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstc3=Take[lstc3~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstc4=Take[lstc4~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
...
fnameA = PerturbativeDataPath<>"perturbative_data_Delta0"<>ToString[\[CapitalDelta]0]<>"_b1"<>ToString[nb[1]]<>"_b2"<>ToString[nb[2]]<>"_f1"<>ToString[nf[1]]<>"_f2"<>ToString[nf[2]]<>"_f3"<>ToString[nf[3]]<>"_f4"<>ToString[nf[4]]<>"_a1"<>ToString[na[1]]<>"_a2"<>ToString[na[2]]<>"_sol"<>ToString[sol]<>".mx";
	...
	cg[1,k_]:= Sum[cg[1,k,n]g^n,{n,-6,nexpn,2}]//Collect[#,g]&;
	cg[2,k_]:= Sum[cg[2,k,n]g^n,{n,-6,nexpn,2}]//Collect[#,g]&;
	cg[3,k_]:= Sum[cg[3,k,n]g^n,{n,-6,nexpn,2}]//Collect[#,g]&;
	cg[4,k_]:= Sum[cg[4,k,n ]g^n,{n,-6,nexpn,2}]//Collect[#,g]&;
	Get[fnameA];
	\[DoubleStruckCapitalF]V0[invsb00=a/@Range[2 cutP+2]];
