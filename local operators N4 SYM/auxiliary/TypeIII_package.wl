...
(*uncomment when TypeIII_script.wls is run in manual mode*)
...
Sequence@@Table[cdown[1,i]->Plist[[2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cdown[2,i]->Plist[[2 + cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cdown[3,i]->Plist[[2 + cutP/2 +cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cdown[4,i]->Plist[[2 + cutP/2 + cutP/2 + cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cup[1,i]->Plist[[2 + 2 cutP + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cup[2,i]->Plist[[2 + 2 cutP + cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cup[3,i]->Plist[[2 + 2 cutP + cutP/2 +cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}],
Sequence@@Table[cup[4,i]->Plist[[2 + 2 cutP + cutP/2 + cutP/2 + cutP/2 + i/2]]//N[#,WP]&,{i,2,cutP,2}]
...
IndepParams={\[CapitalDelta],Table[cdown[1,i],{i,2,cutP,2}],Table[cdown[2,i],{i,2,cutP,2}],Table[cdown[3,i],{i,2,cutP,2}],Table[cdown[4,i],{i,2,cutP,2}],Table[cup[2,i],{i,2,cutP,2}],Table[cup[3,i],{i,2,cutP,2}],Table[cup[4,i],{i,2,cutP,2}]}/.sbgauge//Flatten//DeleteCases[#,0]&;
ParamsInPlist={g,\[CapitalDelta],Table[cdown[1,i],{i,2,cutP,2}],Table[cdown[2,i],{i,2,cutP,2}],Table[cdown[3,i],{i,2,cutP,2}],Table[cdown[4,i],{i,2,cutP,2}],Table[cup[1,i],{i,2,cutP,2}],Table[cup[2,i],{i,2,cutP,2}],Table[cup[3,i],{i,2,cutP,2}],Table[cup[4,i],{i,2,cutP,2}]}//Flatten;
...
lstcDown1=Take[lstcDown1~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstcDown2=Take[lstcDown2~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstcDown3=Take[lstcDown3~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstcDown4=Take[lstcDown4~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];

lstcUp1=Take[lstcUp1~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstcUp2=Take[lstcUp2~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstcUp3=Take[lstcUp3~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
lstcUp4=Take[lstcUp4~Join~Table[0,{cutP/2 - oldcutP}],cutP/2];
...
fnameA = PerturbativeDataPath<>"perturbative_data_Delta0"<>ToString[\[CapitalDelta]0]<>"_b1"<>ToString[nb[1]]<>"_b2"<>ToString[nb[2]]<>"_f1"<>ToString[nf[1]]<>"_f2"<>ToString[nf[2]]<>"_f3"<>ToString[nf[3]]<>"_f4"<>ToString[nf[4]]<>"_a1"<>ToString[na[1]]<>"_a2"<>ToString[na[2]]<>"_sol"<>ToString[sol]<>".mx";
    ...
    cdowng[1, k_] := Sum[cdowng[1, k, n] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cdowng[2, k_] := Sum[cdowng[2, k, n] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cdowng[3, k_] := Sum[cdowng[3, k, n] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cdowng[4, k_] := Sum[cdowng[4, k, n ] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cupg[1, k_] := Sum[cupg[1, k, n] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cupg[2, k_] := Sum[cupg[2, k, n] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cupg[3, k_] := Sum[cupg[3, k, n] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
    cupg[4, k_] := Sum[cupg[4, k, n ] g^n, {n, -6, nexpn, 2}] // Collect[#, g] &;
	Get[fnameA];
