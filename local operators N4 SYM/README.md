...
For the user's convenience, with the authors' permission, we attach the subdirectory *arXiv-1812-09238v1/anc* containing [Perturbative QSC Solver](https://arxiv.org/abs/1812.09238) of Marboe and Volin.
...
To initialise a state with a given $\texttt{State ID}$ you must ensure that there is a *.mx* file with perturbative data that you will need to initialise the numerics. For the 219 states with bare dimenion $\Delta_0\leq 6$, this is already precomputed, and such a file is available in */data/perturbative*. For states with $\Delta_0>6$, please go to the */seed* directory and run the ${\tt Mathematica}$ notebooks there, which access the perturbative data from the [Perturbative QSC Solver](https://arxiv.org/abs/1812.09238) of Marboe and Volin, and transform it to prepare perturbative data compatible with our ${\tt C++}$ implementation.
